...
# --- one consistent case ------------------------------------------------------
# Sentence case for every name and the coach's notes (Javier, 13 Sep 2026) —
# rules in scripts/casing.py. Last, after the corrections and the name matching
# above, which both compare against the coach's original text. CAPITALS ONLY:
# a string whose wording changed stops the build.
sys.path.insert(0, str(pathlib.Path(__file__).resolve().parent))
import casing  # noqa: E402


def cased(old, new, where):
    if old is not None and old.lower() != new.lower():
        raise SystemExit(f"casing changed more than capitals in {where}: {old!r} -> {new!r}")
    return new


for ex in export["exercises"].values():
    ex["name"] = cased(ex["name"], casing.name(ex["name"]), f"exercise {ex['id']}")
for w in export["workouts"]:
    for item in w["items"]:
        item["name"] = cased(item["name"], casing.name(item["name"]), f"{w['key']} {item['letter']} name")
        item["info"] = cased(item["info"], casing.notes(item["info"]), f"{w['key']} {item['letter']} notes")
    for field in ("warmup", "cooldown"):
        if w.get(field):
            w[field] = cased(w[field], casing.notes(w[field]), f"{w['key']} {field}")

# --- one way to write time ----------------------------------------------------
# 45 sec · 2 min · 1:30 min, in names and notes too (Javier, 14 Sep 2026) —
# rules in scripts/units.py. After casing; stops the build if a number changes.
import units  # noqa: E402

for ex in export["exercises"].values():
    ex["name"] = units.text(ex["name"])
for w in export["workouts"]:
    for item in w["items"]:
        item["name"] = units.text(item["name"])
        item["info"] = units.text(item["info"])
    for field in ("warmup", "cooldown"):
        if w.get(field):
            w[field] = units.text(w[field])
...
if units.changes:
    pairs = sorted({(a, b) for a, b in units.changes})
    print(f"  time written as sec / min in {len(units.changes)} strings ({len(pairs)} distinct):")
    for a, b in pairs:
        for la, lb in zip(a.split("\n"), b.split("\n")):
            if la != lb:
                print(f"       {la.strip()!r} → {lb.strip()!r}")
if casing.lowered:
    print(f"  sentence case lowered {len(casing.lowered)} capitalised words — a proper noun among them "
          f"belongs in PROPER in scripts/casing.py:")
    print("       " + ", ".join(sorted(casing.lowered)))
