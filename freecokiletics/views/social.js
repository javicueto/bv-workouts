...
(function () {
    ...
    return '<section class="league" aria-label="League">' + list.map(function (p) {
      ...
      return '<div class="card league__card' + (mine ? " league__card--me" : "") + '">' +
        '<div class="league__top"><span class="avatar' + (mine ? " avatar--me" : "") + '" aria-hidden="true">' + esc(initial(name)) + "</span>" +
          ...
          '<div class="league__status">' + (ICONS[t.icon] || "") + "<span>" + esc(t.name) + "</span></div></div>" +
  ...
  function headline(ctx, it) {
    ...
    // The level's own icon (points.js LADDER); the trophy for an older event without one.
    if (it.kind === "status") return { icon: ICONS[d.icon] || ICONS.trophy, text: who + " reached level " + d.level + ": <b>" + esc(d.name) + "</b>" };
